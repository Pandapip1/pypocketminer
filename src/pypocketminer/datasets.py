...
def determine_global_weights(filestem, positive_cutoff, negative_cutoff):
    y_train = np.load(
        os.path.join(DATA_DIR, f"5-fold-cv/y-train-{filestem}.npy"), allow_pickle=True
    )
...
def pockets_dataset_fold(batch_size, filestem):
    ...
    y_validate = np.load(
        os.path.join(DATA_DIR, f"5-fold-cv/y-val-{filestem}.npy"), allow_pickle=True
    )
    valset = list(zip(X_validate, y_validate))

    X_test = np.load(os.path.join(DATA_DIR, f"5-fold-cv/X-test-{filestem}.npy"))
    y_test = np.load(
        os.path.join(DATA_DIR, f"5-fold-cv/y-test-{filestem}.npy"), allow_pickle=True
    )
    ...
    trainset = tf.data.Dataset.from_generator(
        trainset.__iter__, output_types=output_types
    ).prefetch(3)
    valset = tf.data.Dataset.from_generator(
        valset.__iter__, output_types=output_types
    ).prefetch(3)
    testset = tf.data.Dataset.from_generator(
        testset.__iter__, output_types=output_types
    ).prefetch(3)
...
def pockets_dataset(batch_size, filestem):
    # will have [(xtc,pdb,index,residue,1/0),...]
    X_train = np.load(
        os.path.join(DATA_DIR, f"train-test-split/X-train-{filestem}.npy")
    )
    y_train = np.load(
        os.path.join(DATA_DIR, f"train-test-split/y-train-{filestem}.npy"),
        allow_pickle=True,
    )
    trainset = list(zip(X_train, y_train))

    X_validate = np.load(
        os.path.join(DATA_DIR, f"train-test-split/X-val-{filestem}.npy")
    )
    y_validate = np.load(
        os.path.join(DATA_DIR, f"train-test-split/y-val-{filestem}.npy"),
        allow_pickle=True,
    )
    valset = list(zip(X_validate, y_validate))
    ...
    y_test = np.load(
        os.path.join(DATA_DIR, f"train-test-split/y-test-{filestem}.npy"),
        allow_pickle=True,
    )
    testset = list(zip(X_test, y_test))
...
def simulation_dataset(
    batch_size, filestem, use_tensors=True, y_type="int32", use_lm=False
):
    """
    ...
    y_train = np.load(
        os.path.join(DATA_DIR, f"task2/y-train-{filestem}.npy"), allow_pickle=True
    )
    trainset = list(zip(X_train, y_train))

    trainset = DynamicLoader(
        trainset, batch_size, use_tensors=use_tensors, y_type=y_type, use_lm=use_lm
    )
