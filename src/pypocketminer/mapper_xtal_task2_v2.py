...
# true labels
truelabels_all = np.load(
    f"{upperpath}/bowmanlab/borowsky.jonathan/FAST-cs/protein-sets/all-GVP-project-ligand-resis.npy",
    allow_pickle=True,
)
...
predlabels_all = [
    np.load(f"{predlabelpath}/{i}/full_val_set_y_pred.npy")
    for x, i in enumerate(label_trainschemes)
]
predpraucs_all = [
    np.load(f"{predlabelpath}/{i}/full_val_set_protein_pr_aucs.npy")
    for x, i in enumerate(label_trainschemes)
]
