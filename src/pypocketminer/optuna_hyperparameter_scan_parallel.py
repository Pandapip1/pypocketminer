...
def main_cv(learning_rate, dropout_rate, num_layers, hidden_layers, cv_fold):
    ...
    np.save(
        f"{outdir}/cv_fold{cv_fold}_val_lr_{learning_rate:.5f}_dr_{dropout_rate:.5f}_nl_{num_layers}_hl_{hidden_layers}_cv_loss.npy",
        val_losses,
    )
...
def train_function(
    learning_rate, dropout_rate, num_layers, hidden_layers, cv_fold, results
):
    """
