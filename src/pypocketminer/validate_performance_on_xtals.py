...
if __name__ == "__main__":
    ...
    all_labels = np.load(
        "/project/bowmanlab/borowsky.jonathan/FAST-cs/protein-sets/"
        "all-GVP-project-ligand-resis.npy",
        allow_pickle=True,
    )
        ...
        for epoch in tqdm(range(len(val_files))):
            ...
            if protein_level_performance:
                protein_aucs, protein_pr_aucs = assess_performance(
                    predictions, true_labels, mask
                )
                np.save(
                    os.path.join(nn_dir, f"val_protein_aucs_{epoch}.npy"), protein_aucs
                )
                np.save(
            ...
            np.save(
                os.path.join(nn_dir, f"val_auc_{epoch}.npy"),
                auc_metric.result().numpy(),
            )
            np.save(
                os.path.join(nn_dir, f"val_pr_auc_{epoch}.npy"),
