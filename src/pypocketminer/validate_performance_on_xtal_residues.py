...
if __name__ == "__main__":
    ...
    val_set_apo_ids = np.load(
        "/project/bowmore/ameller/projects/pocket_prediction/data/val_apo_ids.npy"
    )
    ...
    for nn_dir in nn_dirs:
        ...
        for epoch in tqdm(range(len(val_files))):
            ...
            np.save(
                os.path.join(nn_dir, f"val_new_auc_{epoch}.npy"),
                auc_metric.result().numpy(),
            )
            np.save(
                os.path.join(nn_dir, f"val_new_pr_auc_{epoch}.npy"),
        ...
        np.save(
            f"{nn_dir}/new_val_set_cryptosite_subset_auc.npy",
            auc_metric.result().numpy(),
        )
        np.save(
            f"{nn_dir}/new_val_set_cryptosite_subset_pr_auc.npy",
            pr_auc_metric.result().numpy(),
        )
