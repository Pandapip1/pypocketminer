...
from pypocketminer.dataset import simulation_dataset, lookup, abbrev
...
def main():
    ...
    trainset = simulation_dataset(
        BATCH_SIZE, FILESTEM, use_tensors=use_tensors, y_type="float32"
    )
...
if continue_previous_training:
    outdir = (
        previous_nn_dir + f"_refine_feat_method_fpocket_drug_scores_"
        f"{feat_type}_window_{window}_cutoff_{cutoff}_stride_{stride}/"
    )
...
model_path = outdir + "{}_{}"
FILESTEM = (
    f"fpocket-drug-scores-{feat_type}-cutoff-{cutoff}-window-{window}ns-stride-{stride}"
)
print(FILESTEM)
