...
from pypocketminer.optimal_threshold_protein_performance import (
...
if __name__ == "__main__":
    ...
    val_set_apo_ids_with_chainids = np.load(
        "/project/bowmore/ameller/projects/"
    ...
    print(
        f"Total negative residues in val: {np.sum(val_true_labels[val_label_mask] == 0)}"
    )
    print(
        f"Total positive residues in val: {np.sum(val_true_labels[val_label_mask] == 1)}"
    )

    # Load test set input
    test_label_dictionary = np.load(
        "/project/bowmore/ameller/projects/pocket_prediction/data/test_label_dictionary.npy",
        allow_pickle=True,
    ).item()
    test_set_apo_ids_with_chainids = np.load(
        "/project/bowmore/ameller/projects/"
    ...
    test_label_mask = np.zeros(
        [len(test_label_dictionary.keys()), test_l_max], dtype=bool
    )
    for i, l in enumerate(test_label_dictionary.values()):
        test_label_mask[i] = np.pad(l != 2, [[0, test_l_max - len(l)]])

    test_true_labels = np.zeros(
        [len(test_label_dictionary.keys()), test_l_max], dtype=int
    )
    for i, l in enumerate(test_label_dictionary.values()):
    ...
    print(
        f"Total negative residues in test: {np.sum(test_true_labels[test_label_mask] == 0)}"
    )
    print(
        f"Total positive residues in test: {np.sum(test_true_labels[test_label_mask] == 1)}"
    )
    ...
    X_val, S_val, mask_val = process_paths(
        val_set_apo_ids_with_chainids, use_tensors=False
    )
    _, S_val_lm, _ = process_paths(
        val_set_apo_ids_with_chainids, use_tensors=False, use_lm=True
    )

    X_test, S_test, mask_test = process_paths(
        test_set_apo_ids_with_chainids, use_tensors=False
    )
    ...
    print(
        [
            (p, s1, s2)
            for p, s1, s2 in zip(
                test_set_apo_ids_with_chainids,
                np.sum(mask_test.astype(bool) & test_label_mask, axis=1),
                np.sum(test_label_mask, axis=1),
            )
        ]
    )
    # print(np.sum(test_label_mask, axis=1))
    ...
    for nn_dir in nn_dirs:
        ...
        for epoch in tqdm(range(3)):
            ...
            if "lm" in nn_dir:
                predictions = predict_on_xtals(
                    model, nn_path, X_val, S_val_lm, mask_val
                )
            else:
        ...
        np.save(
            os.path.join(nn_dir, "test_protein_performance.npy"), protein_performance
        )

        print(
            [
                (p, round(v, 3))
                for p, v in zip(test_label_dictionary.keys(), protein_performance)
            ]
        )
